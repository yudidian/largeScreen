...
function tagcloud() {
  ...
  function TagCloud(options) {
    ...
    // 鼠标移入
    TagCloud._on(self.box, 'mouseover', () => {
      self.active = true
    })
    // 鼠标移出
    TagCloud._on(self.box, 'mouseout', () => {
      self.active = false
    })

    // 鼠标在内移动
    TagCloud._on(self.keep ? window : self.box, 'mousemove', (ev) => {
      const oEvent = window.event || ev
      const boxPosition = self.box.getBoundingClientRect()
  ...
  TagCloud._set = function (element) {
    if (!TagCloud.boxs.includes(element)) { // ie8不支持数组的indexOf方法
      return true
    }
  }
  ...
  TagCloud.prototype = {
    ...
    update() {
      ...
      const a = -(Math.min(Math.max(-self.mouseY, -self.size), self.size) / self.radius) * self.mspeed
      const b = (Math.min(Math.max(-self.mouseX, -self.size), self.size) / self.radius) * self.mspeed
      ...
      const sc = TagCloud._getSc(a, b)

      for (let j = 0, len = self.items.length; j < len; j++) {
        const rx1 = self.items[j].x
        const ry1 = self.items[j].y * sc[1] + self.items[j].z * (-sc[0])
        const rz1 = self.items[j].y * sc[0] + self.items[j].z * sc[1]
    ...
    _getItems() {
      const self = this
      const items = []
      const element = self.box.children // children 全部是Element
  ...
  return function (options) { // factory
    ...
    const selector = options.selector || '.tagcloud' // 默认选择class为tagcloud的元素
